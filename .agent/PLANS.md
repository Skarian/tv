...
When implementing an executable specification (ExecPlan), do not prompt the user for "next steps"; proceed through the plan milestone by milestone. Keep all sections up to date, add or split entries in the list at every stopping point to affirmatively state the progress made and next steps. The ExecPlan should resolve ambiguities up-front; if you discover an ambiguity or tradeoff not already resolved by the plan, pause and present it to the user as a numbered list (per AGENTS.md), then incorporate the agreed decision into the plan before continuing. Commit frequently.
...
## ExecPlan storage, index, naming, and archival (canonical)

ExecPlans are repo artifacts. They must live inside the repository, be easy to locate, and be recoverable later without relying on chat history.
...
Each ExecPlan is a single markdown file with a sortable, stable name:
...
- `<slug>` is short, kebab-case, and stable (e.g., `auth-refresh`, `billing-invoice-pdf`).
- Avoid “v2/v3” plan sprawl. If direction truly forks, append `__v2` and record the reason in the Decision Log.

### Required plan header (lightweight metadata)

At the top of every ExecPlan, include a short header before `## Purpose / Big Picture`:

- Plan ID: EP-YYYY-MM-DD\_\_slug
- Status: DRAFT | ACTIVE | BLOCKED | DONE | ARCHIVED
- Created: YYYY-MM-DD
- Last Updated: YYYY-MM-DD
- Owner: UNCONFIRMED | <name/role>

Update `Last Updated` whenever you make a meaningful change to the plan.
...
## Skeleton of a Good ExecPlan
    ...
    Include a short header before `## Purpose / Big Picture`:
    ...
    When you revise the plan, append a dated note at the bottom describing what changed and why.

    - (2026-01-26) Changed X because Y.
